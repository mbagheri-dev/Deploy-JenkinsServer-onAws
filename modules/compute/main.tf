...
resource "aws_instance" "jenkins_server" {
   ...
   key_name = aws_key_pair.keyname_kp.key_name
...
# Creating a key pair in AWS called keyname_kp
resource "aws_key_pair" "keyname_kp" {
   # Naming the key keyname_kp
   key_name   = "key-name"

   # Passing the public key of the key pair we created
   public_key = file("${path.module}/key-name.pub")
}
